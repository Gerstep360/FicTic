        ...
        <div class="card p-4 bg-blue-500/10 border-blue-500/20">
            <div class="flex items-start gap-3">
                ...
                <div class="flex-1 text-sm text-slate-300">
                    <span class="font-medium text-blue-400">Editando:</span> 
                    {{ $horario->grupo->materia->nombre }} - Grupo {{ $horario->grupo->nombre_grupo }}
                    ({{ $dias[$horario->dia_semana] }}, {{ $horario->bloque->etiqueta ?? "Bloque {$horario->id_bloque}" }})
                </div>
            </div>
        </div>
        ...
        <form method="POST" action="{{ route('horarios.update', $horario) }}" class="space-y-6">
                    ...
                    <select id="id_grupo" name="id_grupo" required class="input w-full">
                        ...
                        @foreach($grupos as $grupo)
                            <option value="{{ $grupo->id_grupo }}" 
                                {{ old('id_grupo', $horario->id_grupo) == $grupo->id_grupo ? 'selected' : '' }}>
                        ...
                        <select id="id_bloque" name="id_bloque" required class="input w-full" @change="validar()">
                            ...
                            @foreach($bloques as $bloque)
                                <option value="{{ $bloque->id_bloque }}" 
                                    {{ old('id_bloque', $horario->id_bloque) == $bloque->id_bloque ? 'selected' : '' }}>
                        ...
                        <select id="id_aula" name="id_aula" required class="input w-full" @change="validar()">
                            ...
                            @foreach($aulas as $aula)
                                <option value="{{ $aula->id_aula }}" 
                                    {{ old('id_aula', $horario->id_aula) == $aula->id_aula ? 'selected' : '' }}>
                        ...
                        <select id="id_docente" name="id_docente" required class="input w-full" @change="validar()">
                            ...
                            @foreach($docentes as $docente)
                                <option value="{{ $docente->id }}" 
                                    {{ old('id_docente', $horario->id_docente) == $docente->id ? 'selected' : '' }}>
                                    {{ $docente->name }}
                                </option>
                            @endforeach
                        </select>
            ...
            <div class="flex flex-col-reverse sm:flex-row items-stretch sm:items-center gap-3">
                ...
                <button type="submit" class="btn-primary flex-1" :disabled="validando || conflictos.length > 0">
                    ...
                    <span x-show="!validando">Actualizar Horario</span>
